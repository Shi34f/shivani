...
public class UserServiceTest {
	...
	public void getAllDetailsTest() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.get("/user").contentType(MediaType.APPLICATION_JSON_VALUE))
				.andExpect(status().isOk()).andExpect(jsonPath("$", hasSize(3))).andExpect(jsonPath("$[0].userId", is(1)))
				.andExpect(jsonPath("$[0].customerName", is("Shivani"))).andExpect(jsonPath("$[1].userId", is(2)))
				.andExpect(jsonPath("$[1].address", is("UP")));
